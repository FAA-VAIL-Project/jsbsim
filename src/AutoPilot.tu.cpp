...
void AutoPilot::set_throttle_pos(JSBSim::FGFDMExec* FDM)
{
  ...
  if ((aircraftstate.vc <= maxRateSpeed + throttleOffset - throttleHyster) && !throttleOn)
  {
    throttleOn = 1;
  }
  else if ((aircraftstate.vc >= maxRateSpeed + throttleOffset + throttleHyster) && throttleOn)
  {
    throttleOn = 0;
  }
